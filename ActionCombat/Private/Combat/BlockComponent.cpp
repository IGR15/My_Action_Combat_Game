...
bool UBlockComponent::Check(AActor* Opponent)
{
	...
	//this take into account the direction of the opponent and the player if it returns a negative value it means that the player is facing the opponent

	if (Result > 0.0 || !PlayerRef->HasEnoughStamina(StaminaCost)) 
	{ 
		return true; 
	}
